"""
v57: For (64,1024) try a16w8 pg2 instead of pg1 (with fast_mode=False for safety).
v24 used pg2 here and got 34.3us vs v43's 37.3us with pg1. 3us potential savings.
The seed failure in v24 may have been from fast_mode=True; using fast_mode=False here.

Changes from v43:
  - (64,1024): a16w8 pg2 splits=8 (was a16w8 pg1 splits=8)
  - Removed Path B (a16w8 pg1) entirely since no config uses it
"""
...
# Path C: a16w8 pg2 — for (64,1024) AND (256,1024)
A16W8_PG2_CONFIGS = {(64, 1024), (256, 1024)}
A16W8_PG2_SPLITS = {
    (64, 1024): 8,
    (256, 1024): 16,
}
...
def _a16w8_pg2_path(q, kv_data, qo_indptr, kv_indptr, config):
    """Path C: bf16 Q + fp8 KV, page_size=2."""
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
