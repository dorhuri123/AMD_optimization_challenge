"""
v65: MXFP4 Triton with V_CHUNK_D=256 instead of 128.

Change from v43: V_CHUNK_D=256 in MXFP4 path (was 128).
Rationale: With V_CHUNK_D=128, we launch 4 V chunks per (batch, split).
With V_CHUNK_D=256, only 2 chunks but each does more work.
Less program launch overhead, better data reuse within each program.
All other configs unchanged from v43.
"""
...
# v65 CHANGE: V_CHUNK_D=256 instead of 128
# ===============================================================
...
def _mxfp4_path(q, kv_data, kv_indptr, config):
    ...
    BLOCK_N = 64
    V_CHUNK_D = 256  # v65 CHANGE: was 128
    num_v_chunks = V_HEAD_DIM // V_CHUNK_D  # 2 (was 4)
