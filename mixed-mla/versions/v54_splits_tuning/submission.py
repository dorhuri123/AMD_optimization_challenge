"""
v54: Aggressive splits tuning based on dgavriloff's splits=32 success.

Changes from v43:
  1. splits=32 for (256,8192) — was 16, more parallelism may help
  2. splits=32 for (64,8192)  — was 16
  3. splits=32 for (32,8192)  — was 16
  4. splits=12 for (64,1024)  — was 8, try more splits for mid-size
  5. splits=8  for (4,8192)   — keep (already good)
  6. @torch.inference_mode() decorator
  7. Eliminate .item() calls
  8. Optimized dispatch

The key insight: dgavriloff used splits=32 universally and achieved top results.
More splits = more thread blocks = better GPU occupancy on MI300X (304 CUs).
For (256,8192): 256*32=8192 split-blocks vs 256*16=4096, much better CU utilization.
Expected saving: 1-3us on large configs, especially (256,8192) and (64,8192).
"""
...
# ===============================================================
# SPLIT CONFIGS -- TUNED: more aggressive splits
# ===============================================================

_SPLIT_MAP = {
    # MXFP4 Triton (keep small -- these are tiny configs)
    (4, 1024): 4,
    (32, 1024): 4,
    # a16w8 pg1 -- try 12 splits (was 8)
    (64, 1024): 12,
    # a16w8 pg2 -- try 24 splits (was 16)
    (256, 1024): 24,
    # a8w8 pg8 -- aggressive splits for better CU occupancy
    (4, 8192): 8,       # keep (small batch, enough splits)
    (32, 8192): 32,     # was 16 -> 32
    (64, 8192): 32,     # was 16 -> 32
    (256, 8192): 32,    # was 16 -> 32
}
