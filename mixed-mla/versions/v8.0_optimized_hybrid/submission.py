"""
Optimized MLA decode submission -- v8.0 Optimized Hybrid (MXFP4 + AITER)

Based on v5.0 (71.4 us) with the following optimizations:
1. Fused FP8 Q quantization via two Triton kernels (replaces 3-op PyTorch path)
   - _amax_kernel: parallel block reduction -> global amax
   - _quantize_fp8_kernel: read amax, compute scale, quantize in one pass
   Saves ~8 us on bs<=64 by eliminating PyTorch dispatch overhead.
2. Pre-allocated Q FP8 output buffer, scale buffer, and amax scratch buffer
   (avoids torch.empty allocation on every call)
...
# ===============================================================
# AITER DECODE PATH (with fused FP8 quantization)
# ===============================================================

def _aiter_path(q, kv_data, qo_indptr, kv_indptr, config):
    """AITER cached a8w8 path for large configs, using fused FP8 Q quantization."""
