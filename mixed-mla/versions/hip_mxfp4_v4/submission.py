"""
MXFP4 v4: Optimized V loading via LDS + FP8 LUT for MLA decode on gfx950.

Based on v3 (FP8 Q x MXFP4 K Scaled MFMA QK + BF16 MFMA PV).

Key optimization over v3:
  - FP8->BF16 lookup table (256 entries) built in shared memory at kernel start
  - V data loaded cooperatively into LDS with coalesced global reads
  - PV MFMA reads V operands from LDS instead of individual global memory reads
  - Eliminates millions of uncoalesced byte reads per thread
...
// ---------------------------------------------------------------
// MXFP4 v4: Optimized V loading via LDS + FP8 LUT
// ---------------------------------------------------------------

__global__ __launch_bounds__(256, 2)
void mla_mxfp4_v4_splitk(
...
// Torch C++ wrapper
// ---------------------------------------------------------------
torch::Tensor mla_mxfp4_v4_forward(
    torch::Tensor Q_fp8,
    torch::Tensor K_fp4,
    ...
    mla_mxfp4_v4_splitk<<<grid_splitk, block_splitk, smem_bytes>>>(
...
def _try_compile():
    ...
    try:
        ...
        _hip_module = load_inline(
            ...
            functions=["mla_mxfp4_v4_forward"],
            verbose=True,
            extra_cuda_cflags=[
                "-O3",
                "-mcpu=gfx950",
        ...
        _hip_available = True
        print("[HIP MXFP4 v4] Compilation SUCCESS (FP8 LUT + LDS V loading)")
    except Exception as e:
        _hip_available = False
        print(f"[HIP MXFP4 v4] Compilation FAILED: {e}")
        import traceback
        traceback.print_exc()
    return _hip_available
...
def _hip_mxfp4_path(q, kv_data, kv_indptr, config):
    """Primary path: HIP MXFP4 v4 kernel (FP8 QK + LDS-optimized BF16 PV)."""
    ...
    output = _hip_module.mla_mxfp4_v4_forward(
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if _hip_available:
        try:
            ...
            print(f"[HIP MXFP4 v4] Runtime error, falling back to AITER: {e}")

    if _aiter_available:
        return _aiter_fallback(data)

    raise RuntimeError("Neither HIP MXFP4 v4 kernel nor AITER available")
