"""
v40: Safe all-AITER with a16w8 pg1 for kv=1024.

No MXFP4 (fails leaderboard seeds). No pg2 for kv=1024 (fails some seeds).

Path A: a16w8 pg1 for kv=1024 configs (bf16 Q, no quant, safe)
Path B: a16w8 pg2 for kv=8192 medium configs
...
# NO MXFP4 (fails leaderboard seeds)
MXFP4_CONFIGS = set()
MXFP4_KV_SPLITS = {}

# Path A: a16w8 pg1 for kv=1024 (safe, no pg2 risk)
A16W8_PG1_CONFIGS = {(4, 1024), (32, 1024), (64, 1024), (256, 1024)}

# Path B: a16w8 + PAGE_SIZE=2 for kv=8192 medium configs
A16W8_PG2_CONFIGS = {(4, 8192), (32, 8192), (64, 8192)}
...
# ===============================================================
# PATH A2: a16w8 + PAGE_SIZE=1 (for kv=1024, safe)
# ===============================================================
...
def _a16w8_pg1_path(q, kv_data, qo_indptr, kv_indptr, config):
    """a16w8 pg1: bf16 Q (no quant), fp8 KV, page_size=1."""
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    total_kv = batch_size * kv_seq_len
    num_kv_splits = 8 if total_kv <= 4096 else 16
