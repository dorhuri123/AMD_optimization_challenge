"""
v39: Safe all-AITER — no MXFP4 (fails some leaderboard seeds).

Path A: a16w8 pg2 for kv=8192 configs
Path B: a8w8 pg1 for kv=1024 configs (pg2 fails some kv=1024 seeds)
...
# NO MXFP4 — fails some leaderboard seeds
MXFP4_CONFIGS = set()  # empty — never use MXFP4
MXFP4_KV_SPLITS = {}

# Path A: a16w8 + PAGE_SIZE=2 for kv=8192 configs
A16W8_PG2_CONFIGS = {(4, 8192), (32, 8192), (64, 8192)}
# splits: 8 if total_kv<=4096 else 16

# Path B: a8w8 + PAGE_SIZE=1 for kv=1024 configs AND (256,8192)
# kv=1024 uses a8w8 pg1 because pg2 fails some seeds on kv=1024
A8W8_KV1024_CONFIGS = {(4, 1024), (32, 1024), (64, 1024), (256, 1024)}
A8W8_SPLITS = 24
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if (bs, kvlen) in A16W8_PG2_CONFIGS:
        ...
        # All kv=1024 configs + (256,8192) use a8w8 pg1
