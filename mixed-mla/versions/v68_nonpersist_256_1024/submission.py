"""
v68: Based on v43 hybrid, with (256,1024) changed to a8w8 pg1 splits=1.

With num_kv_splits=1, AITER skips the reduce step entirely and writes directly
to the output buffer. This eliminates the reduce kernel launch + intermediate
buffer allocation.
...
Routing:
  ...
  (256,1024): a8w8 pg1 splits=1  <-- CHANGED: was a16w8 pg2 splits=16
...
# Path C: a8w8 pg1 splits=1 — (256,1024), no reduce step
A8W8_PG1_S1_CONFIGS = {(256, 1024)}
A8W8_PG1_S1_SPLITS = {
    (256, 1024): 1,
}
...
# ===============================================================
# PATH C: a8w8 pg1 splits=1 (for (256,1024)) — NO REDUCE STEP
# ===============================================================

def _get_a8w8_pg1_s1_meta(batch_size, kv_seq_len, num_kv_splits, qo_indptr, kv_indptr):
    key = (batch_size, kv_seq_len, num_kv_splits)
    if key not in _a8w8_pg1_s1_meta_cache:
        kv_last_page_len = (kv_indptr[1:] - kv_indptr[:-1]).to(torch.int32)
        total_kv = int(kv_indptr[-1].item())
        kv_indices = torch.arange(total_kv, dtype=torch.int32, device="cuda")
        info = get_mla_metadata_info_v1(
        ...
        _a8w8_pg1_s1_meta_cache[key] = (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_len)
    return _a8w8_pg1_s1_meta_cache[key]


def _a8w8_pg1_s1_path(q, kv_data, qo_indptr, kv_indptr, config):
    """Path C: fp8 Q + fp8 KV, page_size=1, splits=1 for (256,1024).

    With splits=1, AITER skips the reduce kernel entirely and writes
    directly to the output buffer.
    """
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    num_kv_splits = A8W8_PG1_S1_SPLITS[(batch_size, kv_seq_len)]
    ...
    (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_len) = \
        _get_a8w8_pg1_s1_meta(batch_size, kv_seq_len, num_kv_splits, qo_indptr, kv_indptr)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Path C: a8w8 pg1 splits=1 for (256,1024) — no reduce step
    if (bs, kvlen) in A8W8_PG1_S1_CONFIGS:
        return _a8w8_pg1_s1_path(q, kv_data, qo_indptr, kv_indptr, config)
