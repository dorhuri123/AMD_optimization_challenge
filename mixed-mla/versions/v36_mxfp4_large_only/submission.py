"""
v36: Hybrid — v24 optimal AITER + HIP MXFP4 v3 for large kv=8192 configs.
...
Routing:
  ...
  (64, 8192) -> HIP MXFP4 v3, fallback a16w8 pg2 (v24 Path B)
  (256,1024) -> a16w8 pg2    (v24 Path B)
  (256,8192) -> HIP MXFP4 v3, fallback a8w8 pg1  (v24 Path C)
...
# HIP MXFP4 v3 configs — only these two use the HIP kernel (when available)
...
#define FP8_BYTES_PER_MFMA_K  128
#define FP4_BYTES_PER_MFMA_K  64
...
#define SMEM_SCORES_OFFSET_RAW (SMEM_Q_FP8_OFFSET + SMEM_Q_FP8_SIZE)
#define SMEM_SCORES_OFFSET    ((SMEM_SCORES_OFFSET_RAW + 15) & ~15)
...
#define TOTAL_SMEM_BYTES      (SMEM_P_BF16_OFFSET + SMEM_P_BF16_SIZE + 16)
...
void mla_mxfp4_v3_splitk(
    ...
    const unsigned char* __restrict__ K_scale,
    const unsigned char* __restrict__ V_fp8,
    ...
    for (int t_base = 0; t_base < num_tokens; t_base += TOKENS_PER_ITER) {
        ...
        if (wave_tokens > 0) {
            ...
            for (int mfma_idx = 0; mfma_idx < NUM_QK_MFMA; mfma_idx++) {
                int elem_offset = mfma_idx * MFMA_K;
...
def _try_compile():
    ...
    try:
        ...
        _hip_available = True
        print("[v36 HIP MXFP4 v3] Compilation SUCCESS")
    except Exception as e:
        _hip_available = False
        print(f"[v36 HIP MXFP4 v3] Compilation FAILED, using AITER fallbacks: {e}")
        import traceback
        traceback.print_exc()
    return _hip_available
...
def _hip_mxfp4_path(q, kv_data, kv_indptr, config):
    """HIP MXFP4 v3 kernel (FP8 QK + BF16 PV) for large kv=8192 configs."""
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # HIP MXFP4 v3 for (64,8192) and (256,8192)
    if _hip_available and (bs, kvlen) in HIP_MXFP4_CONFIGS:
        try:
            return _hip_mxfp4_path(q, kv_data, kv_indptr, config)
        except Exception as e:
            print(f"[v36 HIP MXFP4 v3] Runtime error for ({bs},{kvlen}), falling back to AITER: {e}")
