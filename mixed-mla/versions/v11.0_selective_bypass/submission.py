"""
Optimized MLA decode submission -- v11.0 Selective AITER Bypass

Per-config routing based on v9 profiling data:
- MXFP4: (4,1024), (4,8192), (32,1024), (64,1024) -- Triton dot_scaled
- AITER BYPASS: (32,8192), (64,8192) -- direct C++ calls, cached logits/attn_lse
  Saves 16-20 us by eliminating torch.empty allocations in Python wrapper.
- AITER WRAPPER: (256,1024), (256,8192) -- standard mla_decode_fwd with cached metadata
  Bypass HURTS bs=256 (+12 us), so we keep the wrapper for those configs.

Carried forward from v9/v8:
...
# ===============================================================
# AITER CACHED METADATA (shared base -- no logits/attn_lse)
# ===============================================================

def _get_cached_meta(bs, nq, nkv, q_dtype, kv_dtype, qo_indptr, kv_indptr, num_kv_splits):
    """Cached metadata for the AITER wrapper path (no logits/attn_lse)."""
...
def _get_cached_bypass_meta(bs, nq, nkv, q_dtype, kv_dtype, qo_indptr, kv_indptr, num_kv_splits):
    ...
    if key not in _bypass_meta_cache:
        ...
        # Cache the intermediate buffers that mla_decode_fwd normally allocates
        # every call -- this is the key optimization of the bypass path.
...
# ===============================================================
# AITER BYPASS PATH -- direct C++ calls (for bs=32,64 kv=8192)
# ===============================================================
...
# ===============================================================
# AITER WRAPPER PATH -- standard mla_decode_fwd (for bs=256)
# ===============================================================
...
def custom_kernel(data: input_t) -> output_t:
    ...
    elif (bs, kvlen) in BYPASS_CONFIGS:
