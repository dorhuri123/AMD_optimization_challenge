"""
v30: v24 with improved Path C — try a16w8 pg2 + higher splits for bs=256,kv=8192.

Path A: MXFP4 Triton (same as v24)
Path B: a16w8 + PAGE_SIZE=2 (same as v24)
Path C: a16w8 + PAGE_SIZE=2 for bs=256,kv=8192 (NEW: skip fp8 Q quant, use paged layout)
  bf16 Q (no quant overhead), fp8 KV, page_size=2, splits=32

Key hypothesis: For bs=256,kv=8192, the Q quantization overhead (~25μs) may not be
worth it when AITER's a16w8 path is better optimized internally.
"""
...
# Path B: a16w8 + PAGE_SIZE=2 (medium configs)
A16W8_PG2_CONFIGS = {(32, 8192), (64, 1024), (64, 8192), (256, 1024)}

# Path C: a16w8 + PAGE_SIZE=2, higher splits for bs=256,kv=8192
# (was a8w8 pg1 splits=24 in v24)
LARGE_SPLITS = 32
...
# ===============================================================
# PATH B/C: a16w8 + PAGE_SIZE=2 (all non-MXFP4 configs)
# ===============================================================
...
def _a16w8_pg2_path(q, kv_data, qo_indptr, kv_indptr, config):
    ...
    # Tuned splits: more splits for largest config
    if total_kv <= 4096:
        num_kv_splits = 8
    elif total_kv <= 65536:
        num_kv_splits = 16
    else:
        # bs=256, kv=8192 => total=2097152
        num_kv_splits = LARGE_SPLITS
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if (bs, kvlen) in MXFP4_CONFIGS:
        ...
        # All non-MXFP4: a16w8 pg2 (including bs=256,kv=8192)
