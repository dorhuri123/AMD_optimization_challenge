"""
MXFP4 v3: FP8 Q x MXFP4 K Scaled MFMA QK + BF16 MFMA PV for MLA decode on gfx950.

Combines:
  1. __builtin_amdgcn_mfma_scale_f32_16x16x128_f8f6f4 with cbsz=0 blgp=4
     (FP8 Q x MXFP4 K — better accuracy than MXFP4 x MXFP4)
  2. v_mfma_f32_16x16x32_bf16 for PV multiplication (replaces scalar FP8 LUT)
...
// ---------------------------------------------------------------
// MXFP4 v3: FP8 Q x MXFP4 K MFMA QK + BF16 MFMA PV Split-K Kernel
// ---------------------------------------------------------------

__global__ __launch_bounds__(256, 2)
void mla_mxfp4_v3_splitk(
    ...
    for (int t_base = 0; t_base < num_tokens; t_base += TOKENS_PER_ITER) {
        ...
        //
        // Strategy: Each thread computes softmax for head=my_head only
        // (since that's the head it accumulates for PV).
        // BUT we need P_bf16 for all heads in smem for the MFMA.
        // So we have all 256 threads cooperate: distribute heads across threads.
        //
        // With 256 threads and 16 heads: 16 threads per head.
        // Use tid / 16 = head (0..15), tid % 16 = worker within head.
        // Each worker finds tile_max over a subset of tokens, then warp-reduce.
        //
        // Actually, simpler approach: just have each thread iterate all heads
        // since tokens_per_iter is small (64 tokens, 16 heads = 1024 values).
        // The softmax is cheap compared to MFMA and global memory loads.
        // ---------------------------------------------------------------
...
// Torch C++ wrapper
// ---------------------------------------------------------------
torch::Tensor mla_mxfp4_v3_forward(
    torch::Tensor Q_fp8,
    torch::Tensor K_fp4,
    ...
    mla_mxfp4_v3_splitk<<<grid_splitk, block_splitk, smem_bytes>>>(
...
def _try_compile():
    ...
    try:
        ...
        _hip_module = load_inline(
            ...
            functions=["mla_mxfp4_v3_forward"],
            verbose=True,
            extra_cuda_cflags=[
                "-O3",
                "-mcpu=gfx950",
        ...
        _hip_available = True
        print("[HIP MXFP4 v3] Compilation SUCCESS (FP8 QK + BF16 PV MFMA)")
    except Exception as e:
        _hip_available = False
        print(f"[HIP MXFP4 v3] Compilation FAILED: {e}")
        import traceback
        traceback.print_exc()
    return _hip_available
...
def _hip_mxfp4_path(q, kv_data, kv_indptr, config):
    """Primary path: HIP MXFP4 v3 kernel (FP8 QK + BF16 PV)."""
    ...
    output = _hip_module.mla_mxfp4_v3_forward(
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if _hip_available:
        try:
            ...
            print(f"[HIP MXFP4 v3] Runtime error, falling back to AITER: {e}")

    if _aiter_available:
        return _aiter_fallback(data)

    raise RuntimeError("Neither HIP MXFP4 v3 kernel nor AITER available")
