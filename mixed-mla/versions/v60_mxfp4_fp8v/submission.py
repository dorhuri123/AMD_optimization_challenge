"""
v60: MXFP4 Triton with V loaded from FP8 instead of bf16.
Currently the Triton kernel loads V from kv_data["bf16"] which is 2x bandwidth.
FP8 V is already available in kv_data["fp8"]. For kv=1024 this saves significant
memory bandwidth on the V load (576 bytes/tok -> 576 bytes but fp8 = half bandwidth).

Changes from v43:
  - Triton kernel loads V from fp8 kv_data, dequants to bf16 in registers
  - Applies to (4,1024) and (32,1024) MXFP4 configs
"""
...
# Path A: MXFP4 Triton — fastest for small kv=1024 configs (now with FP8 V)
...
# ===============================================================
# MXFP4 TRITON KERNEL -- STAGE 1 (with FP8 V loading)
# ===============================================================
...
def _mla_mxfp4_stage1(
    ...
    V_fp8_ptr,        # [total_kv, 576] fp8 (first 512 dims used for V)
    V_scale_ptr,      # [1] f32 — kv_scale for fp8 dequant
    ...
    for tile_idx in range(num_tiles):
        ...
        # Load V from FP8 and dequant to bf16
        ...
        # Dequant: fp8 -> f32, multiply by scale, then to bf16
        v_tile = (v_tile_fp8.to(tl.float32) * v_scale).to(tl.bfloat16)
...
# ===============================================================
# PATH A: MXFP4 TRITON with FP8 V (for (4,1024) and (32,1024))
# ===============================================================
...
def _mxfp4_path(q, kv_data, kv_indptr, config):
    """Path A: MXFP4 dot_scaled with FP8 V loading for small configs."""
    ...
    # Use FP8 KV for V (half the bandwidth of bf16)
    v_fp8_2d = kv_fp8.view(-1, QK_HEAD_DIM)
    ...
    _mla_mxfp4_stage1[grid1](
        ...
        v_fp8_2d,
        kv_fp8_scale,  # V scale for dequant
