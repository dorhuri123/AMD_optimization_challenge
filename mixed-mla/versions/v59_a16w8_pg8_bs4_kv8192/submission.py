"""
v59: For (4,8192) try a16w8 pg8 (bf16 Q, no quant) instead of a8w8 pg8.
Saves ~5us Q quantization overhead. Only 4 batch elements so bf16 Q bandwidth is small.
The AITER kernel handles bf16 Q + fp8 KV natively.

Changes from v43:
  - (4,8192): a16w8 pg8 splits=8 (was a8w8 pg8 splits=8) — skip Q quant
"""
...
# Path B: a16w8 pg1 — safe for (64,1024)
...
# Path E: a16w8 pg8 — bf16 Q + fp8 KV for (4,8192) to skip Q quant
A16W8_PG8_CONFIGS = {(4, 8192)}
A16W8_PG8_SPLITS = {
    (4, 8192): 8,
}

# Path D: a8w8 pg8 — remaining kv=8192 configs
A8W8_PG8_SPLITS = {
    (32, 8192): 16,
    (64, 8192): 16,
    (256, 8192): 16,
...
# ===============================================================
# PATH E: a16w8 pg8 — bf16 Q + fp8 KV for (4,8192) — skip Q quantization
# ===============================================================
...
def _get_a16w8_pg8_meta(batch_size, kv_seq_len, num_kv_splits, qo_indptr, kv_indptr):
    ...
    if key not in _a16w8_pg8_meta_cache:
        ...
        # NOTE: dtype_q=torch.bfloat16 for a16w8
        info = get_mla_metadata_info_v1(
            batch_size, 1, NUM_HEADS, torch.bfloat16, FP8_DTYPE,
            is_sparse=False, fast_mode=False,
            num_kv_splits=num_kv_splits, intra_batch_mode=True,
...
def _a16w8_pg8_path(q, kv_data, qo_indptr, kv_indptr, config):
    """Path E: bf16 Q + fp8 KV, page_size=8 — NO Q quantization."""
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    ...
    # Pass bf16 Q directly — no FP8 quantization needed
    mla_decode_fwd(
        q, kv_buffer_4d, o,
        qo_indptr, kv_indptr_paged, kv_indices, kv_last_page_lens,
        1,
        ...
        logit_cap=0.0,
        num_kv_splits=num_kv_splits,
        q_scale=None,  # No q_scale for bf16 Q
        kv_scale=kv_scale,
        intra_batch_mode=True,
        work_meta_data=wm, work_indptr=wi, work_info_set=wis,
        reduce_indptr=ri, reduce_final_map=rfm, reduce_partial_map=rpm,
...
# ===============================================================
# PATH D: a8w8 pg8 (for kv=8192 configs except (4,8192))
# ===============================================================
...
def _a8w8_pg8_path(q, kv_data, qo_indptr, kv_indptr, config):
    """Path D: fp8 Q + fp8 KV, page_size=8."""
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Path E: a16w8 pg8 for (4,8192) — skip Q quantization
    if (bs, kvlen) in A16W8_PG8_CONFIGS:
        return _a16w8_pg8_path(q, kv_data, qo_indptr, kv_indptr, config)

    # Path D: a8w8 pg8 for remaining kv=8192 configs
