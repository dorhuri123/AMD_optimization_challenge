"""
v20: Optimal hybrid — BF16-Q MXFP4 Triton + Non-persistent AITER

Combines our two proven fastest approaches:
  MXFP4 Triton (bf16 Q, dot_scaled): (4,1024), (4,8192), (32,1024), (64,1024)
    - v17's bf16 Q approach (no Q quantization, saves ~40us vs v5's MXFP4 Q)
    - dot_scaled("bf16", "e2m1") for QK attention
    - bf16 V via tl.dot (NOT FP8 V — v18 proved FP8 V is slower)
    - 3D grid with v_chunks, fixed splits per config

  Non-persistent AITER: (32,8192), (64,8192), (256,1024), (256,8192)
    - v19's approach: mla_decode_fwd WITHOUT work_meta_data
    - AITER auto-computes optimal num_kv_splits internally
    - No persistent metadata overhead

JIT warmup at import time for both paths:
  - AITER Triton JIT (stage2 reduce kernel ~170s compile)
  - MXFP4 Triton kernel compilation
"""
...
# MXFP4 split-K tuning (proven in v12/v17)
MXFP4_KV_SPLITS_MAP = {
    (4, 1024): 4,
    (4, 8192): 16,
    (32, 1024): 4,
...
def _mla_bf16q_mxfp4k_stage1(
    Q_bf16_ptr,       # [batch*16, 576] uint16 (bf16 viewed as uint16)
...
def _warmup_aiter():
    """Pre-warm AITER JIT by running a small dummy non-persistent call.
    Triggers compilation of stage1 ASM + stage2 Triton reduce kernel (~170s).
    """
    bs_warmup = 4
    kv_per_seq = 64
...
def _warmup_mxfp4():
    """Pre-warm MXFP4 Triton kernels by running a tiny dummy call.
    Triggers JIT compilation of _mla_bf16q_mxfp4k_stage1 and _mla_mxfp4_reduce.
    """
    batch_size = 4
    num_kv_splits = 4
    total_kv = batch_size * 64  # 64 KV tokens per seq
    num_v_chunks = V_HEAD_DIM // 128  # 4

    # Dummy Q as bf16 viewed as uint16
    q_uint16 = torch.zeros(batch_size * NUM_HEADS, QK_HEAD_DIM, dtype=torch.uint16, device="cuda")
    q_scale = torch.full((batch_size * NUM_HEADS, NUM_SCALES), 127, dtype=torch.uint8, device="cuda")

    # Dummy KV
    kv_fp4_2d = torch.zeros(total_kv, PACKED_QK, dtype=torch.uint8, device="cuda")
    kv_scale_2d = torch.zeros(total_kv, NUM_SCALES, dtype=torch.uint8, device="cuda")
    v_bf16_2d = torch.zeros(total_kv, QK_HEAD_DIM, dtype=torch.bfloat16, device="cuda")

    # Dummy indptr
    kv_indptr = torch.arange(0, batch_size + 1, dtype=torch.int32, device="cuda") * 64

    # Partial buffers
    partial_o = torch.empty(batch_size, num_kv_splits, NUM_HEADS, V_HEAD_DIM, dtype=torch.float32, device="cuda")
    partial_m = torch.empty(batch_size, num_kv_splits, NUM_HEADS, dtype=torch.float32, device="cuda")
    partial_l = torch.empty(batch_size, num_kv_splits, NUM_HEADS, dtype=torch.float32, device="cuda")
...
def _get_aiter_cache(bs, kv_indptr, device):
    key = bs
    ...
    return _aiter_cache[key]


# ===============================================================
# BF16-Q + MXFP4-K DECODE PATH (from v17)
# ===============================================================
...
# ===============================================================
# NON-PERSISTENT AITER DECODE PATH (from v19)
# ===============================================================

def _aiter_path(q, kv_data, qo_indptr, kv_indptr, config):
    """Non-persistent AITER path. Auto-tunes num_kv_splits internally."""
    ...
    # Cached values
    c = _get_aiter_cache(bs, kv_indptr, q.device)
    o = c["output"]
