"""
v31: v24 but Path C uses a8w8 + PAGE_SIZE=2 for bs=256,kv=8192.

Hypothesis: Paged layout (pg2) with fp8 Q might be faster than pg1 for the
largest config because AITER's persistent kernel can exploit the coalesced
memory access pattern that paging enables.

Path A: MXFP4 Triton (small configs) — same as v24
Path B: a16w8 pg2 (medium configs) — same as v24
Path C: a8w8 pg2 splits=32 (bs=256,kv=8192) — NEW

Also tries 32 splits (vs v24's 24) for more parallelism on large config.
"""
...
MXFP4_KV_SPLITS = {(4, 1024): 4, (4, 8192): 16, (32, 1024): 4}

A16W8_PG2_CONFIGS = {(32, 8192), (64, 1024), (64, 8192), (256, 1024)}

# Path C: a8w8 pg2 for bs=256,kv=8192
A8W8_PG2_SPLITS = 32
PAGE_SIZE_2 = 2
...
_a8w8_pg2_meta_cache: dict = {}
_output_cache: dict = {}


# ===============================================================
# MXFP4 TRITON KERNELS (same as v24)
# ===============================================================

@triton.jit
def _mla_mxfp4_stage1(
    Q_packed_ptr, Q_scale_ptr, K_packed_ptr, K_scale_ptr, V_bf16_ptr,
    Partial_O_ptr, Partial_m_ptr, Partial_l_ptr, kv_indptr_ptr,
    stride_q_packed, stride_q_scale, stride_kv_packed, stride_kv_scale, stride_v_tok,
    stride_po_b, stride_po_s, stride_po_h, stride_ml_b, stride_ml_s, stride_ml_h,
    sm_scale,
...
def _mxfp4_path(q, kv_data, kv_indptr, config):
    ...
    _mla_mxfp4_stage1[grid1](
        ...
        bufs["partial_o"], bufs["partial_m"], bufs["partial_l"], kv_indptr,
        q_packed.stride(0), q_scale.stride(0), kv_fp4_2d.stride(0), kv_scale_2d.stride(0), v_bf16_2d.stride(0),
        ...
        SM_SCALE, BLOCK_N=BLOCK_N, V_CHUNK_D=V_CHUNK_D, NUM_KV_SPLITS=num_kv_splits, NUM_HEADS=NUM_HEADS)
...
# ===============================================================
# PATH C: a8w8 + PAGE_SIZE=2 for bs=256,kv=8192
# ===============================================================
...
def _get_a8w8_pg2_meta(batch_size, kv_seq_len, num_kv_splits, q_dtype, kv_dtype, qo_indptr, kv_indptr):
    key = (batch_size, kv_seq_len, num_kv_splits)
    if key not in _a8w8_pg2_meta_cache:
        seq_lens_kv = kv_indptr[1:] - kv_indptr[:-1]
        num_pages_per_req = (seq_lens_kv + PAGE_SIZE_2 - 1) // PAGE_SIZE_2
        kv_indptr_paged = torch.zeros(batch_size + 1, dtype=torch.int32, device="cuda")
        kv_indptr_paged[1:] = torch.cumsum(num_pages_per_req, dim=0)
        ...
        kv_indices = torch.arange(total_pages, dtype=torch.int32, device="cuda")
        _a8w8_pg2_meta_cache[key] = (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_lens, kv_indptr_paged)
    return _a8w8_pg2_meta_cache[key]

def _a8w8_pg2_path(q, kv_data, qo_indptr, kv_indptr, config):
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    total_kv = batch_size * kv_seq_len
    num_kv_splits = A8W8_PG2_SPLITS

    q_fp8, q_scale = quantize_fp8(q)
    kv_fp8, kv_scale = kv_data["fp8"]

    (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_lens, kv_indptr_paged) = \
        _get_a8w8_pg2_meta(batch_size, kv_seq_len, num_kv_splits,
    ...
    num_pages = total_kv // PAGE_SIZE_2
    kv_buffer_4d = kv_fp8.view(num_pages, PAGE_SIZE_2, NUM_KV_HEADS, kv_fp8.shape[-1])
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if (bs, kvlen) in MXFP4_CONFIGS:
        ...
        # bs=256, kv=8192: a8w8 pg2 with 32 splits
        return _a8w8_pg2_path(q, kv_data, qo_indptr, kv_indptr, config)
