"""
Optimized MLA decode submission -- v18 BF16-Q + MXFP4-K + FP8-V

V loaded from FP8 (1 byte) instead of bf16 (2 bytes) = 50% less V bandwidth.
QK: bf16 Q × MXFP4 K via dot_scaled("bf16", "e2m1") — no Q quant overhead.
V: FP8 cast to bf16, multiply by kv_scale — trivial dequant.

ALL 8 configs use MXFP4+FP8V path (testing if it beats AITER on large configs).
AITER fallback preserved for safety.
"""
...
# ALL configs use MXFP4+FP8V (testing if FP8 V fixes large config regression)
...
def _mla_bf16q_mxfp4k_stage1(
    ...
    V_fp8_ptr,        # [total_kv, 576] uint8 (FP8, first 512 dims used)
    ...
    stride_v_tok,     # stride per V token (576 for FP8 as uint8)
    fp8_v_scale,      # scalar float32: FP8 per-tensor scale for V dequant
    ...
    for tile_idx in range(num_tiles):
        ...
        # V from FP8: Triton natively loads fp8 dtype, casts to f32, applies scale
        vd_offsets = vd_start + tl.arange(0, V_CHUNK_D)
        v_fp8 = tl.load(
            V_fp8_ptr + kv_idx[:, None] * stride_v_tok + vd_offsets[None, :],
            mask=mask_kv[:, None],
            other=0.0,
        )
        # fp8 → f32 (Triton handles format) → scale → bf16 for dot
        v_bf16 = (v_fp8.to(tl.float32) * fp8_v_scale).to(tl.bfloat16)
        acc += tl.dot(p.to(tl.bfloat16), v_bf16, out_dtype=tl.float32)
...
def _mxfp4_path(q, kv_data, kv_indptr, config):
    ...
    # V from FP8: 1 byte per element (half the bandwidth of bf16!)
    # Keep as original fp8 dtype so Triton can bitcast properly
    v_fp8_2d = kv_fp8.view(-1, QK_HEAD_DIM)  # (total_kv, 576) in fp8 dtype
    fp8_v_scale = kv_scale_fp8.item()  # scalar float32
