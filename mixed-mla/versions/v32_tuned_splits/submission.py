"""
v32: v24 with tuned splits and env vars.

Changes from v24:
1. Environment variables for GPU performance
2. Path B splits tuned per-config instead of threshold-based
3. Path C: try splits=32 (vs v24's 24)
4. Pre-allocate all output buffers in first call
"""
...
# Path C: a8w8 pg1 — tuned splits
A8W8_PARAMS = {
    (256, 8192): {"splits": 32},  # was 24 in v24
}
...
def _a16w8_pg2_path(q, kv_data, qo_indptr, kv_indptr, config):
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    params = A16W8_PG2_PARAMS[batch_size, kv_seq_len]
    num_kv_splits = params["splits"]
...
def _get_a8w8_meta(batch_size, kv_seq_len, num_kv_splits, q_dtype, kv_dtype, qo_indptr, kv_indptr):
    key = (batch_size, kv_seq_len, num_kv_splits)
    if key not in _a8w8_meta_cache:
        ...
        get_mla_metadata_v1(
            ...
            NUM_HEADS // NUM_KV_HEADS, NUM_KV_HEADS, True,
            wm, wis, wi, ri, rfm, rpm,
            page_size=PAGE_SIZE_1, kv_granularity=max(PAGE_SIZE_1, 16),
            max_seqlen_qo=1, uni_seqlen_qo=1, fast_mode=False,
            max_split_per_batch=num_kv_splits, intra_batch_mode=True,
...
def _a8w8_path(q, kv_data, qo_indptr, kv_indptr, config):
    batch_size = config["batch_size"]
    kv_seq_len = config["kv_seq_len"]
    params = A8W8_PARAMS[batch_size, kv_seq_len]
    num_kv_splits = params["splits"]
    q_fp8, q_scale = quantize_fp8(q)
    kv_fp8, kv_scale = kv_data["fp8"]
    (wm, wi, wis, ri, rfm, rpm, kv_indices, kv_last_page_len) = \
        _get_a8w8_meta(batch_size, kv_seq_len, num_kv_splits, q_fp8.dtype, kv_fp8.dtype, qo_indptr, kv_indptr)
    out_key = (batch_size, NUM_HEADS)
    if out_key not in _output_cache:
        _output_cache[out_key] = torch.empty((batch_size, NUM_HEADS, V_HEAD_DIM), dtype=torch.bfloat16, device=q.device)
    o = _output_cache[out_key]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    elif (bs, kvlen) in A16W8_PG2_PARAMS:
